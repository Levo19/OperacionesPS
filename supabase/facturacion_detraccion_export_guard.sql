-- SNAPSHOT VIVO tras revisión 500x (emitir_comprobante) — patch aplicado por _apply_*.js
...
begin
  ...
  if p_detraccion and (coalesce(p_exportacion,false) or not (p_tipo = 1 and v_total > 700)) then
    raise exception 'DETRACCION_SOLO_FACTURA_B2B: la detracción aplica a facturas nacionales > S/700 (no exportación)'; end if;
