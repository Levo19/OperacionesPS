-- get_dashboard sin pases cancelados (2026-08-01) — generado del vivo + parche
  ...
  select coalesce(jsonb_agg(o order by o->>'__ord' desc),'[]'::jsonb) into v_ops from (
    select jsonb_build_object(
      ...
      'manifiesto', coalesce((select jsonb_agg(jsonb_build_object(
            ...
            'id_contactoPase',coalesce(m.contacto_pase_id,''),'adicionales',_adic_txt(m.adicionales),'tarifa_base',m.tarifa_base)
