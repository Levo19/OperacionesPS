-- FORMA DE PAGO contado/crédito (2026-08-02) — generado del vivo + parche
...
declare v_num int; v_id text; v_existing comprobantes; v_total numeric; v_grav numeric; v_igv numeric; v_exo numeric;
        ...
        v_err text; v_hash text; v_fpago text; v_venc date; v_real boolean; v_errlow text; v_obs text; v_doc text; v_export numeric; v_inaf numeric; v_detr numeric;
begin
  perform _req_staff();
  ...
  select coalesce(sum(round((i->>'cantidad')::numeric * (i->>'precio')::numeric, 2)), 0) into v_total from jsonb_array_elements(p_items) i;
  ...
  insert into comprobantes(tipo,serie,numero,moneda,cliente_doc_tipo,cliente_doc,cliente_nombre,cliente_email,cliente_tel,
      exonerado,es_exportacion,total_gravada,total_exonerada,total_exportacion,total_inafecta,total_igv,total,items,estado,enlace_pdf,enlace_xml,enlace_cdr,qr,hash,codigo_barras,medio_pago,forma_pago,credito_vencimiento,detraccion,detraccion_total,
...
AS $function$ declare v jsonb; begin
  ...
  from (select id, tipo, serie, numero, cliente_nombre, cliente_doc, cliente_doc_tipo, moneda,
          ...
          items, estado, enlace_pdf, qr, hash,
