-- Cortesía TC / operación gratuita (2026-08-02) — generado del vivo + parche
CREATE OR REPLACE FUNCTION public._nf_items(p_items jsonb, p_exonerado boolean, p_exportacion boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE sql
 IMMUTABLE
AS $function$
  -- IGV por RESTA: total de línea = round(precio*cant,2) (lo que paga el cliente);
  -- valor de venta = round(total/1.18,2) (gravado); IGV = total - valor. Garantiza:
  --   subtotal + igv = total  y  precio_unitario*cantidad = total  y  valor_unitario*cantidad = subtotal (exactos).
  -- valor/precio unitario a 6 decimales para que NubeFact recomponga sin desvío con cantidad>1.
  select coalesce(jsonb_agg(jsonb_build_object(
    'unidad_de_medida','ZZ','codigo','S','descripcion', descripcion,
    'cantidad', cant, 'valor_unitario', round(sub/cant,6), 'precio_unitario', round(tot/cant,6), 'subtotal', sub,
    'tipo_de_igv', case af when 'inafecto' then 9 when 'exonerado' then 8 when 'exportacion' then 16 when 'gratuito' then 11 else 1 end,
    'igv', igv, 'total', tot
  )),'[]'::jsonb)
  from (
    select descripcion, cant, af, tot,
...
begin
  ...
  insert into comprobantes(tipo,serie,numero,moneda,cliente_doc_tipo,cliente_doc,cliente_nombre,cliente_email,cliente_tel,
      exonerado,es_exportacion,total_gravada,total_exonerada,total_exportacion,total_inafecta,total_igv,total,total_gratuita,items,estado,enlace_pdf,enlace_xml,enlace_cdr,qr,hash,codigo_barras,medio_pago,forma_pago,credito_vencimiento,detraccion,detraccion_total,
    ...
    values(p_tipo,p_serie,v_num,coalesce(p_moneda,'PEN'),nullif(p_cliente_doc_tipo,''),nullif(p_cliente_doc,''),
      p_cliente_nombre,nullif(p_cliente_email,''),nullif(p_cliente_tel,''),coalesce(p_exonerado,false),coalesce(p_exportacion,false),v_grav,v_exo,v_export,v_inaf,v_igv,v_total,coalesce(v_gratis,0),p_items,
