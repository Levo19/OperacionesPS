...
function generarCierreDelDia() {
    // ── GUARD: verificar datos pendientes antes de cerrar ─────────────────
    let pendingWarn = [];
    if (pendingPostRequests > 0)
        pendingWarn.push(`• ${pendingPostRequests} solicitud(es) aún en tránsito`);
    let queuedCaja = (window.cajaData || []).filter(c => c._syncing || c._queued);
    if (queuedCaja.length)
        pendingWarn.push(`• ${queuedCaja.length} transacción(es) de caja pendiente(s) de confirmar`);
    try {
        let offQ = JSON.parse(localStorage.getItem(_OFFLINE_Q_KEY) || '[]');
        if (offQ.length) pendingWarn.push(`• ${offQ.length} acción(es) en cola offline (sin conexión)`);
    } catch(e) {}
    if (pendingWarn.length) {
        if (!confirm('⚠️ HAY DATOS PENDIENTES:\n\n' + pendingWarn.join('\n') + '\n\nSe recomienda esperar la sincronización antes de cerrar.\n¿Continuar de todas formas?')) return;
    }
    ...
    let cajaAll   = (window.cajaData || []).filter(c => esFechaHoy(c.timestamp) && !c._syncing && !c._queued);
    ...
    // ── Totales globales ──────────────────────────────────────────────────
    const METODOS_VIRTUAL = ['Yape','Plin','Transferencia','Tarjeta'];
    let ingTotal=0, salTotal=0, ingEfectivo=0, salEfectivo=0, ingVirtual=0, salVirtual=0;
    let byMetodo = {};
    caja.forEach(c => {
        let m = parseFloat(c.monto)||0;
        let met = c.metodo_pago || 'Efectivo';
        let isPase = met==='Pase_Canje'||met==='Pase / Canje';
        if (isPase) return;
        let esI = _esIngresoCaja(c);
        if (esI) ingTotal+=m; else salTotal+=m;
        if (!byMetodo[met]) byMetodo[met]={ing:0,sal:0};
        if (esI) byMetodo[met].ing+=m; else byMetodo[met].sal+=m;
        if (met==='Efectivo') { if(esI) ingEfectivo+=m; else salEfectivo+=m; }
        else if (METODOS_VIRTUAL.includes(met)) { if(esI) ingVirtual+=m; else salVirtual+=m; }
    });
    let netoEfectivo = ingEfectivo - salEfectivo;
    let netoVirtual  = ingVirtual  - salVirtual;
    let netoTotal    = netoEfectivo + netoVirtual;
    let totalPaxGlobal = 0;
    ops.forEach(op => { totalPaxGlobal += parseInt(op.ocupados)||0; });
    let totalBotes = ops.filter(o=>o.estado==='En_Viaje'||o.estado==='Cerrada').length;

    // ── Agencias ──────────────────────────────────────────────────────────
    let agMap = {};
    let _seenAg = new Set();
    ops.forEach(op => (op.manifiesto||[]).forEach(m => {
        if (m.tipo !== 'Agencia') return;
        if (m.id && _seenAg.has(m.id)) return;
        if (m.id) _seenAg.add(m.id);
        let pax=parseInt(m.pax)||0; if(!pax) return;
        let key = m.nombreContacto||m.contacto||'—';
        if(!agMap[key]) agMap[key]={pax:0,cobrar:0,cobrado:0,id:m.contacto};
        agMap[key].pax+=pax; agMap[key].cobrar+=parseFloat(m.monto)||0;
    }));
    caja.filter(c=>c.categoria==='Cobro').forEach(c => {
        let key = resolveNombre(c.id_contacto);
        if(agMap[key]) agMap[key].cobrado+=parseFloat(c.monto)||0;
    });

    // ── Comisionados ──────────────────────────────────────────────────────
    let comMap = {};
    let _seenCom = new Set();
    ops.forEach(op => (op.manifiesto||[]).forEach(m => {
        if (m.tipo !== 'Comisionado') return;
        if (m.id && _seenCom.has(m.id)) return;
        if (m.id) _seenCom.add(m.id);
        let pax=parseInt(m.pax)||0; if(!pax) return;
        let cobrado=parseFloat(m.monto)||0;
    ...
    caja.filter(c=>c.categoria==='Pagos'||c.categoria==='Pago_Comisionado'||c.categoria==='Pago Comisionado').forEach(c=>{
        let key=resolveNombre(c.id_contacto);
        if(!comMap[key]) comMap[key]={pax:0,cobrado:0,base:0,comision:0,pagado:0};
        comMap[key].pagado+=parseFloat(c.monto)||0;
    });
    let totalComision=0, totalComPagado=0;
    Object.values(comMap).forEach(d=>{totalComision+=d.comision;totalComPagado+=d.pagado;});

    // ── Pases ─────────────────────────────────────────────────────────────
    let pasesAliado = {};
    pases.filter(p=>(p.aliadoId||'').trim()&&!(p.id_agencia_comprada||'').trim()).forEach(p=>{
        let k=resolveNombre(p.aliadoId);
        if(!pasesAliado[k]) pasesAliado[k]={out:[],in:[]};
        pasesAliado[k].out.push({pax:parseInt(p.pax)||0,origen:p.nombreOrigen||'—',monto:parseFloat(p.monto)||0,
            hora:p.timestamp?new Date(p.timestamp).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}):'—'});
    });
    ops.forEach(op=>(op.manifiesto||[]).forEach(m=>{
        if(m.tipo!=='Aliado(PaseIn)'&&m.tipo!=='Aliado') return;
    ...
    Object.values(pasesAliado).forEach(d=>{
        totalPaxPaseOut+=d.out.reduce((s,x)=>s+x.pax,0);
        totalPaxPaseIn+=d.in.reduce((s,x)=>s+x.pax,0);
    });

    // ── Compras a agencias (pases convertidos) ────────────────────────────
    let comprasMap = {};
    pases.filter(p=>(p.id_agencia_comprada||'').trim()).forEach(p=>{
        let agId=(p.id_agencia_comprada||'').trim();
        let agInfo=contactos.find(c=>c.id===agId);
        let agNombre=agInfo?agInfo.nombre:agId;
        let monto=parseFloat(p.monto_comprado)||0;
        let pagado=(window.cajaData||[]).some(c=>(c.id_movimiento||'')===p.id&&c.categoria==='Pago Agencia');
        if(!comprasMap[agNombre]) comprasMap[agNombre]={pax:0,monto:0,pagado:0,pendiente:0,items:[]};
        comprasMap[agNombre].pax+=parseInt(p.pax)||0;
        comprasMap[agNombre].monto+=monto;
        if(pagado) comprasMap[agNombre].pagado+=monto; else comprasMap[agNombre].pendiente+=monto;
        comprasMap[agNombre].items.push({pax:parseInt(p.pax)||0,monto,pagado,origen:p.nombreOrigen||'—'});
    });

    // ── Por operador ──────────────────────────────────────────────────────
    let opsList = [...new Set(caja.map(c=>c.operador).filter(Boolean))];
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:'Helvetica Neue',Arial,sans-serif;font-size:11px;color:#1a1a2e;background:#f0f2f5;}
    .page{width:210mm;min-height:297mm;background:white;margin:0 auto;padding:13mm 14mm;}
    .no-print{background:#1e293b;color:white;text-align:center;padding:10px;position:sticky;top:0;z-index:99;font-size:12px;}
    .no-print button{background:#56070c;color:white;border:none;padding:7px 20px;border-radius:8px;font-weight:900;cursor:pointer;margin:0 5px;font-size:12px;}
    @media print{body{background:white;}.no-print{display:none;}.page{padding:10mm 12mm;}}
    @page{size:A4;margin:8mm;}

    /* Hero */
    .hero{background:linear-gradient(135deg,#56070c 0%,#8b0c14 100%);border-radius:14px;padding:18px 22px;color:white;margin-bottom:16px;}
    .hero-title{font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.12em;opacity:.75;margin-bottom:12px;}
    .hero-grid{display:grid;grid-template-columns:1fr 1fr 1.4fr;gap:12px;}
    .hero-card{background:rgba(255,255,255,.13);border-radius:10px;padding:12px 14px;border:1px solid rgba(255,255,255,.2);}
    .hero-card.main{background:rgba(255,255,255,.22);border:2px solid rgba(255,255,255,.5);}
    .hero-label{font-size:9px;font-weight:700;opacity:.7;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;}
    .hero-val{font-size:24px;font-weight:900;line-height:1;}
    .hero-sub{font-size:9px;opacity:.65;margin-top:5px;}

    /* Métricas top */
    .metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px;}
    .metric{border-radius:10px;padding:10px 12px;text-align:center;border:1px solid;}
    .metric-label{font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:.07em;margin-bottom:3px;}
    .metric-val{font-size:20px;font-weight:900;}

    /* Secciones */
    .section{margin-bottom:18px;page-break-inside:avoid;}
    .sec-header{display:flex;align-items:center;gap:8px;padding:7px 12px;border-radius:8px 8px 0 0;color:white;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.07em;}
    .sec-body{border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;overflow:hidden;}

    /* Tablas */
    table{width:100%;border-collapse:collapse;font-size:10px;}
    thead tr{background:#f8fafc;}
    th{padding:6px 8px;font-size:8.5px;font-weight:900;text-transform:uppercase;letter-spacing:.05em;color:#64748b;text-align:left;border-bottom:2px solid #e2e8f0;}
    td{padding:5px 8px;border-bottom:1px solid #f1f5f9;vertical-align:middle;}
    tr:last-child td{border-bottom:none;}
    tr:hover td{background:#fafafa;}
    .tr-subtotal td{background:#f8fafc;font-weight:900;border-top:2px solid #e2e8f0;}

    /* Badges */
    .badge{display:inline-block;padding:2px 7px;border-radius:20px;font-size:8.5px;font-weight:900;}
    ...
    /* Método badge */
    .met-efectivo{background:#dcfce7;color:#15803d;}
    .met-yape{background:#ede9fe;color:#6d28d9;}
    .met-plin{background:#ccfbf1;color:#0f766e;}
    .met-transf{background:#dbeafe;color:#1d4ed8;}
    .met-tarjeta{background:#fce7f3;color:#9d174d;}

    /* Bote card */
    .bote-header{padding:9px 14px;color:white;display:flex;justify-content:space-between;align-items:center;}
    .bote-footer{background:#f8fafc;padding:6px 12px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid #e2e8f0;}
    `;

    // ── Helpers de renderizado ─────────────────────────────────────────────
    const tbl = (head, body) => `<table>${head}${body}</table>`;
    const thead = (...cols) => `<thead><tr>${cols.map(([t,a])=>`<th style="text-align:${a||'left'}">${t}</th>`).join('')}</tr></thead>`;
    const tbody = rows => `<tbody>${rows||'<tr><td colspan="99" style="color:#94a3b8;text-align:center;padding:12px;">Sin registros.</td></tr>'}</tbody>`;
    const tr = (cells, cls='') => `<tr class="${cls}">${cells}</tr>`;
    const td = (v, a='left', extra='') => `<td style="text-align:${a};${extra}">${v}</td>`;
    const badge = (t, cls) => `<span class="badge ${cls}">${t}</span>`;
    const money = (v, color='') => `<b style="color:${color||'inherit'}">${S(v)}</b>`;
    const metBadge = met => {
        let cls = met==='Efectivo'?'met-efectivo':met==='Yape'?'met-yape':met==='Plin'?'met-plin':met==='Transferencia'?'met-transf':met==='Tarjeta'?'met-tarjeta':'b-gray';
        let ico = met==='Efectivo'?'💵':met==='Yape'?'📱':met==='Plin'?'📲':met==='Transferencia'?'🏦':met==='Tarjeta'?'💳':'💰';
        return `<span class="badge ${cls}">${ico} ${met}</span>`;
    };
    const secHeader = (ico, title, color) => `<div class="sec-header" style="background:${color}">${ico} ${title}</div>`;
    const progBar = (val, max, color) => {
        let p = pct(val,max);
        return `<div class="prog-wrap"><div class="prog-bar" style="width:${p}%;background:${color}"></div></div>`;
    };

    // ═══════════════════════════════════════════════════════════════════════
    // ── HERO: Lo que recibe la jefa ───────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let metVirtualRows = METODOS_VIRTUAL.map(met => {
        let d = byMetodo[met]||{ing:0,sal:0};
        if(d.ing+d.sal===0) return '';
        return `<span style="font-size:9px;opacity:.75;">${metBadge(met).replace('<span class="badge ','<span style="font-size:9px;" class="badge ')} +${S(d.ing)} / -${S(d.sal)}</span> `;
    }).join('');

    let heroHtml = `
    <div class="hero">
        <div class="hero-title">💼 Resumen para la Jefatura — ${hoy} · ${horaCorte}</div>
        <div class="hero-grid">
            <div class="hero-card">
                <div class="hero-label">💵 Efectivo a recibir</div>
                <div class="hero-val" style="color:${netoEfectivo>=0?'#86efac':'#fca5a5'}">${S(netoEfectivo)}</div>
                <div class="hero-sub">Ingresos ${S(ingEfectivo)} · Salidas ${S(salEfectivo)}</div>
            </div>
            <div class="hero-card">
                <div class="hero-label">📱 Virtual a recibir</div>
                <div class="hero-val" style="color:${netoVirtual>=0?'#93c5fd':'#fca5a5'}">${S(netoVirtual)}</div>
                <div class="hero-sub">${metVirtualRows||'Sin movimientos'}</div>
            </div>
            <div class="hero-card main">
                <div class="hero-label">⭐ TOTAL A RECIBIR</div>
                <div class="hero-val">${S(netoTotal)}</div>
                <div class="hero-sub">${totalBotes} bote(s) · ${totalPaxGlobal} PAX · ${opsList.length} operador(es)</div>
            </div>
        </div>
    </div>`;
    ...
    // ── MÉTRICAS RÁPIDAS ──────────────────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let pendComTotal = Object.values(comMap).reduce((s,d)=>s+Math.max(0,d.comision-d.pagado),0);
    let pendComprasTotal = Object.values(comprasMap).reduce((s,d)=>s+d.pendiente,0);
    let pendAgTotal = Object.values(agMap).reduce((s,d)=>s+Math.max(0,d.cobrar-d.cobrado),0);
    let metricsHtml = `
    <div class="metrics">
        <div class="metric" style="background:#f0fdf4;border-color:#86efac;color:#15803d;">
            <div class="metric-label">Ingresos</div>
            <div class="metric-val">${S(ingTotal)}</div>
        </div>
        <div class="metric" style="background:#fef2f2;border-color:#fca5a5;color:#dc2626;">
            <div class="metric-label">Salidas</div>
            <div class="metric-val">${S(salTotal)}</div>
        </div>
        <div class="metric" style="background:#fff7ed;border-color:#fed7aa;color:#c2410c;">
            <div class="metric-label">⚠️ Pendiente cobrar</div>
            <div class="metric-val">${S(pendAgTotal)}</div>
        </div>
        <div class="metric" style="background:#fdf4ff;border-color:#e9d5ff;color:#7e22ce;">
            <div class="metric-label">⚠️ Pendiente pagar</div>
            <div class="metric-val">${S(pendComTotal+pendComprasTotal)}</div>
        </div>
    </div>`;

    // ═══════════════════════════════════════════════════════════════════════
    // ── A. ANÁLISIS DE CAJA POR MÉTODO ───────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let cajaMetRows = Object.entries(byMetodo).map(([met,d])=>{
        let neto=d.ing-d.sal;
        return tr(
            td(metBadge(met)) +
            td(money(d.ing,'#16a34a'),'right') +
            td(money(d.sal,'#dc2626'),'right') +
            td(money(neto,neto>=0?'#0f172a':'#dc2626'),'right')
        );
    }).join('') + tr(
        td('<b>TOTAL</b>') +
        td(money(ingTotal,'#16a34a'),'right') +
        td(money(salTotal,'#dc2626'),'right') +
        td(money(netoTotal,netoTotal>=0?'#56070c':'#dc2626'),'right'),
        'tr-subtotal'
    );

    let cajaDetalleRows = [...caja].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(c=>{
        let m=parseFloat(c.monto)||0;
        let esI=_esIngresoCaja(c);
        ...
        let contactNombre=c.id_contacto?(/^CON-00/i.test(c.id_contacto)?(c.comentarios||'').replace(/^\[.\] ?/,'').trim():resolveNombre(c.id_contacto)):'';
        return tr(
            td(hora,'left','color:#64748b;') +
            td(contactNombre||cat) +
            td(`<span style="font-size:9px;color:#94a3b8;">${cat}</span>`) +
            td(c.operador||'—','left','color:#64748b;') +
            td(metBadge(c.metodo_pago||'Efectivo')) +
            td(`<b style="color:${esI?'#16a34a':'#dc2626'}">${esI?'↑ +':'↓ -'}${S(m)}</b>`,'right')
        );
    }).join('');

    let cajaSec = `
    <div class="section">
        ${secHeader('📊','Análisis de Caja','#1e40af')}
        <div class="sec-body">
            <div style="padding:10px 12px;background:#eff6ff;border-bottom:1px solid #bfdbfe;">
                <b style="font-size:10px;color:#1e40af;">Resumen por método de pago</b>
            </div>
            ${tbl(thead(['Método'],['Ingresos','right'],['Salidas','right'],['Neto','right']), tbody(cajaMetRows))}
            <div style="padding:10px 12px;background:#f8fafc;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;">
                <b style="font-size:10px;color:#475569;">Detalle cronológico</b>
            </div>
            ${tbl(thead(['Hora'],['Contacto / Descripción'],['Categoría'],['Operador'],['Método'],['Monto','right']), tbody(cajaDetalleRows))}
        </div>
    </div>`;
    ...
    let operadoresSec = '';
    if (opsList.length) {
        let opCards = opsList.map(op=>{
            let txsOp=caja.filter(c=>c.operador===op);
            let ingOp=0,salOp=0;
            txsOp.forEach(c=>{ let m=parseFloat(c.monto)||0; if(_esIngresoCaja(c)) ingOp+=m; else salOp+=m; });
            let netoOp=ingOp-salOp;
            let pctOp=pct(ingOp,ingTotal);
            let rowsOp=txsOp.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(c=>{
                let m=parseFloat(c.monto)||0; let esI=_esIngresoCaja(c);
                let cat=c.categoria==='Varios'?'Varios · '+(c.comentarios||'').replace(/^\[.\] ?/,'').substring(0,28):c.categoria.replace(/_/g,' ');
                let hora=c.timestamp?new Date(c.timestamp).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}):'—';
                return tr(td(hora,'left','color:#64748b')+td(cat)+td(metBadge(c.metodo_pago||'Efectivo'))+td(`<b style="color:${esI?'#16a34a':'#dc2626'}">${esI?'+':'-'}${S(m)}</b>`,'right'));
            }).join('');
            return `<div style="margin-bottom:10px;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
                <div style="background:#1e293b;color:white;padding:9px 14px;display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-weight:900;font-size:12px;">👤 ${op}</span>
                    <span style="font-size:9px;opacity:.8;">${txsOp.length} movs. · Ingresos ${S(ingOp)} · Salidas ${S(salOp)} · <b style="color:${netoOp>=0?'#86efac':'#fca5a5'}">Neto ${S(netoOp)}</b></span>
                </div>
                <div style="padding:8px 14px;background:#f8fafc;border-bottom:1px solid #e2e8f0;">
                    <div style="display:flex;justify-content:space-between;font-size:9px;color:#64748b;margin-bottom:4px;">
                        <span>Participación en ingresos totales</span><span>${pctOp}%</span>
                    </div>
                    ${progBar(ingOp,ingTotal,'#3b82f6')}
                </div>
                ${tbl(thead(['Hora'],['Categoría'],['Método'],['Monto','right']),tbody(rowsOp))}
            </div>`;
        }).join('');
        operadoresSec = `
        <div class="section">
            ${secHeader('👤','Detalle por Operador','#1e293b')}
            <div class="sec-body" style="padding:10px;">${opCards}</div>
        </div>`;
    }
    ...
    // ── C. BOTES (manifiesto) ─────────────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let botesCards = ops.map(op=>{
        let totalOp=(op.manifiesto||[]).reduce((s,m)=>s+(parseFloat(m.monto)||0),0);
        let lanzado=op.estado==='En_Viaje'||op.estado==='Cerrada';
        let bgHead=lanzado?'#56070c':'#6b7280';
        let estadoBadge=lanzado?badge('✅ ZARPÓ','b-green'):badge('❌ NO ZARPÓ','b-red');
        let movRows=(op.manifiesto||[]).map(m=>{
            let nombre=m.nombreContacto||m.contacto||'—';
            let TIPO_LABELS={'Aliado(PaseIn)':'Pase ↓In','Aliado(PaseOut)':'Pase ↑Out','Comisionado':'Comisionado','Agencia':'Agencia','Libre':'Libre','Directo':'Libre'};
            let tipoLabel=TIPO_LABELS[m.tipo]||m.tipo||'—';
            let tipoCls=m.tipo&&m.tipo.includes('Aliado')?'b-purple':m.tipo==='Comisionado'?'b-orange':m.tipo==='Agencia'?'b-blue':'b-gray';
            let isAliado=m.tipo&&m.tipo.includes('Aliado');
            let montoStr=isAliado?badge('PASE','b-purple'):`<b style="color:#15803d">${S(m.monto)}</b>`;
            let pagoSt=_calcPagoEstado(m);
            let cobroChip=pagoSt.cobrable?(pagoSt.estado==='pagado_completo'?badge('✓ Cobrado','b-green'):pagoSt.estado==='pagado_parcial'?badge('⚡ Parcial','b-orange'):badge('⏳ Pendiente','b-red')):'';
            return tr(td(nombre)+td(badge(tipoLabel,tipoCls),'center')+td(m.pax||0,'center')+td(montoStr,'right')+td(cobroChip,'center'));
        }).join('')||`<tr><td colspan="5" style="color:#94a3b8;text-align:center;padding:10px;font-style:italic;">Sin pasajeros registrados.</td></tr>`;
        return `<div style="margin-bottom:12px;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;page-break-inside:avoid;">
            <div class="bote-header" style="background:${bgHead};">
                <div>
                    <span style="font-weight:900;font-size:14px;">⛵ ${op.bote}</span>
                    <span style="margin-left:10px;font-size:9px;opacity:.8;">${op.hora_salida||'—'}</span>
                </div>
                <div style="font-size:9px;opacity:.85;text-align:right;">
                    ${op.ocupados}/${op.capacidad} PAX &nbsp;·&nbsp; Cap: ${op.capitan||'—'} &nbsp;·&nbsp; Guía: ${op.guia||'—'}
                </div>
            </div>
            ${tbl(thead(['Pasajero / Contacto'],['Tipo','center'],['PAX','center'],['Monto','right'],['Cobro','center']),`<tbody>${movRows}</tbody>`)}
            <div class="bote-footer">
                ${estadoBadge}
                <span style="font-weight:900;font-size:11px;color:#15803d;">Total facturado: ${S(totalOp)}</span>
            </div>
        </div>`;
    }).join('')||'<p style="color:#94a3b8;padding:12px;">Sin operaciones.</p>';
    let botesSec = `
    <div class="section">
        ${secHeader('⛵','Manifiesto por Bote','#56070c')}
        <div class="sec-body" style="padding:10px;">${botesCards}</div>
    </div>`;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    let pasesSec = '';
    if(totalPaxPaseOut+totalPaxPaseIn>0){
        let paseRows=Object.entries(pasesAliado).map(([aliado,d])=>{
            let sumOut=d.out.reduce((s,x)=>s+x.pax,0);
            let sumIn=d.in.reduce((s,x)=>s+x.pax,0);
            let saldo=sumIn-sumOut;
            let saldoChip=saldo===0?badge('Tablas','b-gray'):saldo>0?badge(`+${saldo} favor`,'b-green'):badge(`${saldo} debe`,'b-red');
            let detOut=d.out.map(x=>`↑ ${x.pax} pax · ${x.origen} (${x.hora})`).join('<br>')||'—';
            let detIn=d.in.map(x=>`↓ ${x.pax} pax`).join('<br>')||'—';
            return tr(td(`<b>${aliado}</b>`)+td(detOut)+td(detIn)+td(`${sumOut}`,'center')+td(`${sumIn}`,'center')+td(saldoChip,'center'));
        }).join('');
        pasesSec=`
        <div class="section">
            ${secHeader('🤝','Análisis de Pases','#7e22ce')}
            <div class="sec-body">
                <div style="padding:8px 14px;background:#fdf4ff;border-bottom:1px solid #e9d5ff;display:flex;gap:20px;font-size:10px;">
                    <span>📤 Total enviados: <b>${totalPaxPaseOut} PAX</b></span>
                    <span>📥 Total recibidos: <b>${totalPaxPaseIn} PAX</b></span>
                    <span>⚖️ Saldo neto: <b style="color:${totalPaxPaseIn-totalPaxPaseOut>=0?'#16a34a':'#dc2626'}">${totalPaxPaseIn-totalPaxPaseOut>=0?'+':''}${totalPaxPaseIn-totalPaxPaseOut} PAX</b></span>
                </div>
                ${tbl(thead(['Aliado'],['Enviados (detalle)'],['Recibidos'],['PAX Out','center'],['PAX In','center'],['Saldo','center']),tbody(paseRows))}
            </div>
        </div>`;
    }

    // ═══════════════════════════════════════════════════════════════════════
    // ── E. COMPRAS A AGENCIAS ─────────────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let comprasSec = '';
    if(Object.keys(comprasMap).length>0){
        let totalComprasMonto=Object.values(comprasMap).reduce((s,d)=>s+d.monto,0);
        let totalComprasPend=Object.values(comprasMap).reduce((s,d)=>s+d.pendiente,0);
        let comprasRows=Object.entries(comprasMap).map(([ag,d])=>{
            let chip=d.pendiente>0?badge(`⚠️ Debe ${S(d.pendiente)}`,'b-red'):badge('✅ Al día','b-green');
            return tr(
                td(`<b>${ag}</b>`)+
                td(d.pax,'center')+
                td(money(d.monto),'right')+
                td(money(d.pagado,'#16a34a'),'right')+
                td(chip,'center')
            );
        }).join('')+tr(td('<b>TOTAL</b>')+td('','center')+td(money(totalComprasMonto),'right')+td('','right')+td(totalComprasPend>0?badge(`⚠️ Pendiente ${S(totalComprasPend)}`,'b-red'):badge('✅ Todo pagado','b-green'),'center'),'tr-subtotal');
        comprasSec=`
        <div class="section">
            ${secHeader('🏢','Compras de PAX a Agencias','#0369a1')}
            <div class="sec-body">
                ${tbl(thead(['Agencia'],['PAX','center'],['Monto Compra','right'],['Pagado','right'],['Estado','center']),tbody(comprasRows))}
            </div>
        </div>`;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    let agenciasSec = '';
    if(Object.keys(agMap).length>0){
        let agRows=Object.entries(agMap).map(([ag,d])=>{
            let pend=Math.max(0,d.cobrar-d.cobrado);
            let chip=pend>0?badge(`⚠️ Debe ${S(pend)}`,'b-red'):badge('✅ Cobrado','b-green');
            let barColor=pend>0?'#ef4444':'#22c55e';
            return tr(
                td(`<b>${ag}</b>`)+
                td(d.pax,'center')+
                td(money(d.cobrar),'right')+
                td(money(d.cobrado,'#16a34a'),'right')+
                td(`<div style="width:70px;display:inline-block;">${progBar(d.cobrado,d.cobrar,barColor)}</div>`,'center')+
                td(chip,'center')
            );
        }).join('');
        agenciasSec=`
        <div class="section">
            ${secHeader('🏬','Análisis de Agencias','#0f766e')}
            <div class="sec-body">
                ${tbl(thead(['Agencia'],['PAX','center'],['A cobrar','right'],['Cobrado','right'],['Avance','center'],['Estado','center']),tbody(agRows))}
            </div>
        </div>`;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    let comSec = '';
    if(Object.keys(comMap).length>0){
        let comRows=Object.entries(comMap).map(([nombre,d])=>{
            let pend=Math.max(0,d.comision-d.pagado);
            let chip=pend>0?badge(`⚠️ Debe ${S(pend)}`,'b-red'):badge('✅ Pagado','b-green');
            return tr(
                td(`<b>${nombre}</b>`)+
                td(d.pax,'center')+
                td(money(d.cobrado),'right')+
                td(money(d.base,'#64748b'),'right')+
                td(money(d.comision,'#d97706'),'right')+
                td(money(d.pagado,'#16a34a'),'right')+
                td(chip,'center')
            );
        }).join('')+tr(td('<b>TOTAL</b>')+td('','center')+td('','right')+td('','right')+td(money(totalComision,'#d97706'),'right')+td(money(totalComPagado,'#16a34a'),'right')+td(totalComision-totalComPagado>0?badge(`⚠️ Debe ${S(totalComision-totalComPagado)}`,'b-red'):badge('✅ Al día','b-green'),'center'),'tr-subtotal');
        comSec=`
        <div class="section">
            ${secHeader('💼','Análisis de Comisionados','#b45309')}
            <div class="sec-body">
                ${tbl(thead(['Comisionado'],['PAX','center'],['Cobrado','right'],['Base','right'],['Comisión','right'],['Pagado','right'],['Estado','center']),tbody(comRows))}
            </div>
        </div>`;
    }
    ...
    // ── HTML FINAL ────────────────────────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    let htmlCierre = `<!DOCTYPE html><html lang="es"><head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Cierre del Día · ${hoy}</title>
    <style>${css}</style></head><body>
    <div class="no-print">
        <span>📄 Cierre del Día · ${hoy} · ${horaCorte}</span>
    ...
    <div class="page">
        ...
        <!-- CABECERA -->
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;padding-bottom:10px;border-bottom:3px solid #56070c;">
            <div>
                <div style="font-size:20px;font-weight:900;color:#56070c;letter-spacing:.05em;">CIERRE DEL DÍA</div>
                <div style="font-size:11px;color:#64748b;margin-top:2px;">${hoy} &nbsp;·&nbsp; Hora de corte: <b>${horaCorte}</b> &nbsp;·&nbsp; Por: <b>${myOpName||'Sistema'}</b></div>
            </div>
            <div style="text-align:right;">
                <div style="font-size:28px;font-weight:900;color:#56070c;">${S(netoTotal)}</div>
                <div style="font-size:9px;color:#94a3b8;text-transform:uppercase;letter-spacing:.07em;">Total neto del día</div>
            </div>
        </div>
        ...
        <!-- PIE -->
        <div style="margin-top:20px;padding-top:10px;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;font-size:9px;color:#94a3b8;">
            <span>OperacionesPS · Sistema de Operaciones Turismo Marítimo</span>
