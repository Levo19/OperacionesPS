...
| D1 | **Contacto duplicado por error del admin (07-sep-2026):** `VALERY LILI BERTONY` (CON-79, aliado) era la misma persona que `BERTONY TOURS` aliado (CON-28); su único pase MOV-1001742 (PaseOut 3 pax, origen ALMORA TOURS CON-12, cobro TX-1001020 S/75) quedaba fuera del balance de Bertony | absorción por DATOS, no por código: `contacto_pase_id` CON-79→CON-28 en el movimiento (origen, monto y cobro intactos) + CON-79 `activo=false` con 0 referencias restantes. Registrado en `reparaciones`. **Receta para duplicados:** verificar refs en movimientos (contacto/pase/compra), caja_operador, reservas, saldos_iniciales → reasignar → desactivar (nunca borrar) |
| R7 | **Pases con origen COMISIONADO en S/0** (ADRIAN y TAXI LIMA 08-ago, SOBRINO DE JUANCHO 28-jul; reporte dueño 09-sep) aunque el operador puso el Total | causa doble: (1) `crear_reserva` **descartaba `p_monto`** (el muelle exige Total en la reserva y el RPC no lo insertaba → `reservas.monto` NULL en TODAS las post-cutover; las 220 con monto son legado GAS) y al pasar/abordar la reserva el front manda `monto=0`; (2) `_cargo_origen` y `tg_default_tarifa` solo cubrían Agencia/Libre → comisionado sin monto manual = 0. Fix (`supabase/_apply_reserva_monto_comisionado.js`): `crear_reserva` persiste monto · `_cargo_origen` + trigger cubren **Comisionado** (precio_defecto×pax; el manual > 0 gana; aliado sigue 0) · backfill de los 3 pases con el **Cobro real ligado** (50/125/75). Regla: comisionado paga la tarifa base, su comisión es lo que cobre por encima — S/0 nunca |
| R8 | **Caja del muelle por operador** (regla dueño 09-sep): el operador ve SOLO sus cobros/pagos; el admin del muelle (`equipo_accesos.muelle='admin'`: Luis, Patricia, Daphne) y el panel PS ven todo | OPS `renderCaja` filtra `caja_operador.operador == myOpName` (case-insensitive) salvo `window._esAdminMuelle` (expuesto por el shim); el cruce de comisionados pagados usa TODA la caja del día (un pago de otro operador sigue siendo pago). Cabecera «Mi caja · nombre» vs «Balance del turno · todos». Solo front: `get_dashboard.movimientos_dia` sigue completo porque los chips pagado/cobrado lo necesitan. OPS v65 / sw ops-v104 |
